...
Loop override ON should make currently-active cues follow the global loop button.

This script:
- Generates a short WAV.
- Starts AudioEngine.
- Plays a cue with loop_enabled=False.
- Turns on global loop and then enables loop override while the cue is active.
- Verifies the cue stays active well past its natural duration.

Run:
  venv/Scripts/python.exe manual_test_loop_override_active_cues.py
"""
...
def main() -> int:
    wav_path = Path("manual_loop_override_active_cues.wav").absolute()
    if wav_path.exists():
        try:
            wav_path.unlink()
        except Exception:
    ...
    try:
        ...
        # Enable override while cue is active; it should now loop.
        time.sleep(0.05)
        print(f"[TRANSPORT] Enabling loop override at +{time.perf_counter() - t0:.3f}s")
        ...
        if not active:
            print("[FAIL] Cue is not active; loop override did not apply to active cue")
            return 2

        print("[PASS] Cue stayed active past natural duration (override + global loop applied)")
        return 0
